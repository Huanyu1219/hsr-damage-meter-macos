...
状态：用户于 2026-09-13 明确选择“复用现有 Socket.IO（推荐）”。
本决定覆盖原 CODEX_SPEC.md 中必须先新增自定义 WebSocket Publisher 的实施顺序，原始规范保留原文。

## 证据

- 用户已通过 Method 2 将官方 veritas.dll 更名为 xluau.dll，成功加载和显示。
...
现有 Rust/Swift v1 模型保留为 M0 历史产物；不假装现有服务符合 v1。
...
用户确认不加载不卡、隐藏浮窗仍卡。复用网络接口不等于解决既有采集/日志开销。
本地游戏 config.ini 为 4.5.0，而 release 声明为 4.3.54+，已观察到枚举名称不匹配及技能空名错误。
接收端必须标记此版本组合未验证；新角色正确性优先核验，不能在 App 中推测和补造漏失事件。
性能方面安排日志级别受控 A/B 验证，见 PERFORMANCE_DIAGNOSIS.md 与独立候选补丁。
