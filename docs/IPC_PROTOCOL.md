...
以 CODEX_SPEC.md 第 5–7、44 节为基础。M0 只定义线协议；以下传输行为在 M1/M2 实现。
...
- sequence 为 1…Int64.max；进程生命周期内严格递增，跨战斗不重置。缺号允许，重复/倒序不能被误算。
  M0 解码器无状态；顺序检查与重连状态机在 M1/M2 实现。进程重启重新 hello 并允许新序列。
...
party_update 视为当前队伍完整快照，空数组有效；具体来源映射在 M1 对照上游验证。
...
M1 必须定义溢出诊断与会话不完整处理，不得悄悄丢事件后宣称总伤害准确；不在 M0 承诺重放。
连接 hello、断线、重连和多客户端排序由 M1/M2 验证。本轮尚无 WebSocket 服务器。
